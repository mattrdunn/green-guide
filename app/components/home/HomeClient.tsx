...
export default function HomeClient({ plants }: { plants: PlantCardData[] }) {
    ...
    const filtered = useMemo(() => {
        ...
        return plants.filter(
            (plant) =>
                (!category || category.matches(plant.tags)) &&
                plantMatchesQuery(plant, needle),
        );
    }, [plants, query, filter]);
    ...
    return (
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-16 px-2">
                ...
                <GreenCard className="rounded-4xl p-8 sm:p-14 dark:from-emerald-950 dark:via-emerald-900/95 dark:to-emerald-950">
                    <div className="space-y-8">
                        ...
                        <form
                            ...
                            className="flex w-full items-center gap-3 rounded-full border border-white/25 bg-emerald-950/60 p-2 pl-5 shadow-lg shadow-emerald-950/30 backdrop-blur transition focus-within:border-white/50 sm:p-2.5 sm:pl-6"
                        >
                            <svg
                                aria-hidden="true"
                                viewBox="0 0 24 24"
                                className="h-5 w-5 shrink-0 fill-none stroke-white/60 stroke-2"
                            >
                            ...
                            <input
                                ...
                                className="w-full bg-transparent py-2 text-base text-white placeholder:text-white/50 focus:outline-none"
                            />
                            <button
                                type="submit"
                                className="shrink-0 rounded-full bg-white px-6 py-2.5 text-sm font-semibold text-emerald-950 transition hover:bg-lime-100"
                            >
                                {t('hero.searchButton')}
                            </button>
                        </form>
            ...
            {/* While searching, drop the static category cards so the filtered
                grid sits directly under the hero — otherwise the results are a
                full screen away, especially on mobile. */}
            {!searching && (
                            ...
                            >
                                ...
                                <div className="p-5">
                                    <h3 className="flex items-center gap-2 font-semibold text-stone-900 dark:text-white">
                                        ...
                                        />
                                        {t(
                                            `categories.items.${category.id}.title`,
                                        )}
                                    </h3>
            ...
            <section id="common-plants" className="scroll-mt-16 space-y-8">
                <div className="space-y-2">
                    ...
                    <h2 className="font-serif text-3xl sm:text-4xl">
                        {searching
                            ? t('plants.searchTitle', { query: query.trim() })
                            : t('plants.title')}
                    </h2>
                </div>
                {filtered.length > 0 ? (
